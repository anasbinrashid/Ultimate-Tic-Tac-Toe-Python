...
class UltimateTicTacToe:
    def __init__(self):
        # Initialize logging
        self.log_to_file("New game started")
        self.root = tk.Tk()
        self.root.title("Ultimate Tic-Tac-Toe Championship")
        self.root.configure(bg="#1a1a2e")
        self.root.minsize(650, 700)  # Set minimum window size        
        # Game state
        ...
        self.control_frame = tk.Frame(self.main_frame, bg=self.bg_color, pady=15, padx=10, bd=2, relief=tk.RIDGE)
        self.control_frame.pack(side=tk.TOP, fill=tk.X, pady=10)
        
        # Mode selection
        self.mode_label = tk.Label(self.control_frame, text="MODE:", bg=self.bg_color, fg="#e94560", 
                                font=("Arial", 11, "bold"))
        ...
        self.mode_menu.config(bg="#0f3460", fg="white", activebackground="#e94560", 
                            activeforeground="white", font=("Arial", 10), width=12,
                            highlightthickness=1, highlightbackground="#e94560")
        self.mode_menu["menu"].config(bg="#0f3460", fg="white", activebackground="#e94560", 
        ...
        self.reset_button.pack(side=tk.RIGHT, padx=15)        
        
        # Status frame
        self.status_frame = tk.Frame(self.main_frame, bg="#0f3460", pady=12, bd=2, relief=tk.RIDGE)
        ...
        self.status_label = tk.Label(self.status_frame, textvariable=self.status_var, 
                                bg="#0f3460", fg="#4ecca3", font=("Arial", 16, "bold"))
        self.status_label.pack(expand=True, pady=5)
                
        # Game board frame
        self.game_frame = tk.Frame(self.main_frame, bg=self.bg_color)
        self.game_frame.pack(expand=True, pady=15)
        self.log_to_file(f"Mode: {self.player_mode}")
        ...
        for i in range(3):
            ...
            for j in range(3):
                ...
                board_frame = tk.Frame(self.game_frame, bg=self.board_color, padx=6, pady=6,
                                    highlightbackground="#e94560", highlightthickness=2, bd=0)
                board_frame.grid(row=i, column=j, padx=8, pady=8)
                ...
                for m in range(3):
                    ...
                    for n in range(3):
                        btn = tk.Button(board_frame, text=' ', width=3, height=1, 
                                    ...
                                    bg="#0f3460", fg="white", disabledforeground="white",
                                    activebackground="#1e5f74", bd=0,
                                    command=lambda i=i, j=j, m=m, n=n: self.make_move(i, j, m, n))
                        btn.grid(row=m, column=n, padx=2, pady=2)
    ...
    def log_to_file(self, message):
        ...
        with open("ultimate_tictactoe_log.txt", "a") as log_file:
            log_file.write(f"[{timestamp}] {message}\n")
    
    def change_mode(self, mode):
        self.player_mode = mode
        self.log_to_file(f"Game mode changed to: {mode}")
        self.reset_game()
    
    def change_difficulty(self, difficulty):
        self.log_to_file(f"AI difficulty changed to: {difficulty}")
        self.ai_difficulty = difficulty
    
    def reset_game(self):
        # Reset game state
        self.log_to_file("Game reset")
        self.board = [[[[' ' for _ in range(3)] for _ in range(3)] for _ in range(3)] for _ in range(3)]
        self.small_board_status = [[' ' for _ in range(3)] for _ in range(3)]
        self.current_player = 'X'
    ...
    def execute_move(self, i, j, m, n):
        ...
        # Log the move
        self.log_to_file(f"Player {self.current_player} moved at big board ({i},{j}), small position ({m},{n})")
        # Update the button
        ...
        button.config(text=self.current_player, 
                    ...
                    fg="#ffffff",
                    font=('Helvetica', 16, 'bold'),
                    relief=tk.RAISED if self.current_player == 'X' else tk.SUNKEN,
                    state=tk.DISABLED)        
        ...
        if small_win != ' ':
            ...
            # Log small board win
            if small_win == 'D':
                self.log_to_file(f"Small board ({i},{j}) ended in a draw")
            else:
                self.log_to_file(f"Player {small_win} won small board ({i},{j})")
        ...
        if big_win:
            ...
            self.winner = big_win
            if big_win == 'D':
                self.log_to_file("Game Over: It's a Draw!")
                self.status_var.set("Game Over: It's a Draw!")
            else:
                self.log_to_file(f"Game Over: Player {big_win} Wins!")
    ...
    def make_ai_move(self):
        ...
        if move:
            i, j, m, n = move
            self.log_to_file(f"AI ({self.current_player}) selected move: big board ({i},{j}), small position ({m},{n})")
            self.execute_move(i, j, m, n)
    ...
    def backtrack(self, assignment):
        ...
        for value in self.get_domain(var):
            ...
            if self.consistent(var, value, assignment):
                ...
                # Forward checking
                if self.forwardchecking(var, value):
                    # Recursive call
